...
The `core.whitespace` configuration variable allows you to define what 'git
diff', 'git apply' and 'git rebase --whitespace=*' should consider whitespace
errors for all paths in the project (See linkgit:git-config[1]).  This
attribute gives you finer control per path.

Set::

	Notice all types of potential whitespace errors known to Git, except
	`tab-in-indent`.  The tab width is taken from the value of the
