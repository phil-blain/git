...
+
See also INTERACTIVE MODE and INCOMPATIBLE OPTIONS below.
...
* No-op invocations: When the range to rebase is already directly a descendant of `<upstream>`  (find a way to word that better),
  the merge backend will not create new commits, whereas the apply backend
  will always create new commits (i.e. with new committer date and committer ident).
  It is as if the apply backend defaults to --force-rebase
  (at least when I try with 'git rebase --apply @{u}).
...
- Repeat the last two steps until your working tree is clean.  To reuse the
  commit message from the original commit you are splitting, you can use
