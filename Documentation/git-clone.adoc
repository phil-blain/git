...
`--reference[-if-able] <repository>`::
	...
	If `--recurse-submodules` is given, set
	`submodule.alternateLocation` to `superproject` such
	that each submodule will have an alternate set up to
	point to the object directory of the corresponding
	submodule in the reference repository.
	When using the `--reference-if-able`, a non existing
	directory is skipped with a warning instead of aborting
	the clone, and if `--recurse-submodules` is given,
	missing submodule directories are similarily skipped.
+
