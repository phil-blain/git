		...
		struct object_id *result_oid)
{
	...
	if (repo_get_oid(o->repo, o->remote_ref, &remote_oid)) {
		/*
	...
	/* Find merge bases */
	bases = repo_get_merge_bases(o->repo, local, remote);
	if (!bases) {
		base_oid = null_oid();
		       ...
		       struct object_id *result_oid)
{
	...
	const char *buffer = repo_get_commit_buffer(o->repo,
						    partial_commit, NULL);
	...
	repo_unuse_commit_buffer(o->repo, partial_commit, buffer);
	if (o->verbosity >= 4)
